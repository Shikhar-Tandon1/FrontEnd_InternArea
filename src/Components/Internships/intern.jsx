...
const [InternData,setInternData]=useState([])
  ...
  useEffect(()=>{
    const fetchData= async()=>{
        try {
        const response= await axios.get(`https://backend-internarea-1dye.onrender.com/api/internship`)
...
<Link to={`/detailInternship?q=${data._id}`} className='mt-10'><button id='viewButtons' className='bg-transparent text-blue-500'>{t("View In Details")}</button></Link>
   </div>
  </div>
  </div>
