...
useEffect(()=>{
  const fetchData= async()=>{
      try {
      const response= await axios.get(`https://backend-internarea-1dye.onrender.com/api/job`)
...
<label htmlFor="pro">{t("Profile.base")}</label>
<input type="text" id='pro'  value={serachCategory} onChange={handleCategoryChange} className='profile border-2 mr-3 w-full' placeholder={t("Profile.placeholder")}/>
<label htmlFor="loc">{t("Location.base")}</label>
<input type="text" id='loc'  value={searchLoaction}  onChange={handleCategoryLocationChange} className='location border-2  -ml-8 w-full' placeholder={t("Location.placeholder")}/>
</div>
...
<div className="search-2"><div className="search-container">
  <label htmlFor="ex ">{t("Experience.base")}</label>
  <input type="text" id='ex' placeholder={t("Experience.placeholder")} />
  ...
  <div className="all-internships">
    <div className=" show show2 flex justify-center">
      <p id='filter-ico' className='filterico text-center' >{t("Filter")}<i class="bi bi-funnel  text-blue-400" onClick={showDiv}></i>   </p>
...
<div className="flex text-sm justify-between">
  ...
  <p className='mt-3'> <i class="bi bi-calendar-check-fill"></i> {t("Experience.base")} <br />
  {data.Experience}</p>

  <p className='mt-3'>  <i class="bi bi-cash"></i>   {t("Salary")} <br /> {data.CTC}</p>
   </div>
   </div>
...
<button id='viewButtons' className='bg-transparent text-blue-500'>{t("View In Details")}</button></Link>
   </div>
  </div>
  </div>
...
<input type="text" id='pro'  value={serachCategory} onChange={handleCategoryChange} className='profile border-2 mr-3 w-full' placeholder={t("Profile.placeholder")}/>
<label htmlFor="loc">{t("Location.base")}</label>
<input type="text" id='loc'  value={searchLoaction}  onChange={handleCategoryLocationChange} className='location border-2 mt-10  -ml-8 w-full' placeholder={t("Location.placeholder")}/>
</div>
<div className=" preferences mt-8 flex flex-col">
<div className="flex mt-3 ml-3 mr-3">
  <input type="checkbox" name="wfh" id="whf"  className='mr-2 ml-3'/>
